// если дебаг-сообщения отсеиваются, то getAValue() даже не вызовется и не выполнится ни один из операторов "<<"
//LOG_DEBUG << "Variable a value is " << getAValue();
 //===================================================================
//
// logger.h - logger
//
...
//#endif // __LOGGER_H
/*
Т.е., если есть дефайн LOG_DISABLED, например, то будет подставлен код if(true); else ...
Оптимизатор его выкинет.
Если же в рантайме уровень лога будет отфильтровывать сообщение, то опять же ничего не выполнится, т.к. дефайн раскроется в if(!logEnabled(Debug)); else...

Буфер временный, для каждого потока создаётся с вызовом первого оператора "LOG <<" и уничтожается сразу после первой же ";", т.е. после вызова всех "<<" применённых к одному LOG.

Допустим, что мы делает отладочное сообщение:
LOG_ERROR << "lanuda" << "da da da";
 

Оно раскроется в
if(!::logger::logEnabled<LOG_IMPLEMENTATION>(::logger::Error));
else ::logger::logError<LOG_IMPLEMENTATION>() << "lanuda" << "da da da";
 

LogError — функция, которая создаёт временный объект типа LOG_IMPLEMENTATION через вызов конструктора
LOG_IMPLEMENTATION(Private::SingletonHolder<typename LOG_IMPLEMENTATION::LogType>::Instance(), Error);
 

Вместо LOG_IMPLEMENTATION может быть любой тип из тех, что я сделал для примера
  typedef StreamLog<SingleCOUTLog> SingleStreamCOUTLog;
  typedef StreamLog<SafeCOUTLog> SafeStreamCOUTLog;
  typedef StreamLog<SingleTraceLog> SingleStreamTraceLog;
  typedef StreamLog<SingleFileLog> SingleStreamFileLog;
...
Т.е. создаётся временный объект шаблонного класса StreamLog. Смотрим его реализацию. В конструкторе он создаёт
std::ostringstream m_strBuffer;
 

в этот буфер всё и пишется через опреатор <<
    template <typename T>
      std::ostream& operator<<(const T& val)
    {
      m_strBuffer << val;
...
После того, как будет вызван последний оператор <<, вызовется деструктор временного объекта
    ~StreamLog()
    {
      if(!m_strBuffer.bad())
      {
...
Как видим, вызывается единственный метод write, в который передан локальный для каждого отдельного потока буфер. Таким образом ничего смертельного не произойдёт, если один поток выполнит << "lanuda" и управление перейдёт в другой поток, т.к. у каждого потока будет свой временный локальный объект у каждого из которого есть свой буфер.
//----------------------------------------------------------------
Да, пример можно было упростить, чтобы показать только идею.
Что мне нравится с оператором "<<", так это то, что если лог фильтруется, то ничто среди операторов "<<" не будет вычисляться, в отличие о варианта аля printf. Кроме того, проще жизнь с размером буфера — он будет расти по необходимости. С вариантом printf многие просто делают фиксированный размер буфера, который может легко переполниться, этим грешат даже коллеги из Trolltech. На мой взгляд, в перспективе оператор << должен выполняться быстрее, чем форматированный вывод, но сейчас на практике реализация оператора << наверняка использует что-то из sprintf, поэтому есть некоторый оверхэд. Ну и последний аргумент — это то, что можно для каждого класса написать метод для вывода в std::ostream и писать в лог объекты своих классов:
class Base
{
public:
    virtual ~Base() {}
    ...
    virtual std::ostream& toStream(std::ostream& os) const = 0; // наследники должны реализовать
};

std::ostream& operator <<(std::ostream& os, const Base& b)
{
    return b.toStream(os); // используем подходящий toStream
}
